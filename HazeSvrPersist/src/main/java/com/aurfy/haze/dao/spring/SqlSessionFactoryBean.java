...
public class SqlSessionFactoryBean implements FactoryBean<SqlSessionFactory>, InitializingBean, ApplicationListener<ApplicationEvent> {
  ...
  private Map<Class<? extends Enum<?>>, Class<? extends TypeHandler<?>>> typeHandlerMap ;
   ...
   * @since 1.0.1
   *
   * @param typeHandlers Type handler Map
   */
  public void setTypeHandlerMap(Map<Class<? extends Enum<?>>, Class<? extends TypeHandler<?>>> typeHandlerMap) {
  ...
  protected SqlSessionFactory buildSqlSessionFactory() throws IOException {
    ...
    if (this.typeHandlerMap != null) {
    	...
    	for(Class<?> clz : typeHandlerMap.keySet()) {
    		configuration.getTypeHandlerRegistry().register(clz, typeHandlerMap.get(clz));
            if (logger.isDebugEnabled()) {
              logger.debug("Registered type handler: '" + typeHandlerMap.get(clz) + "'");
            }
    	}
    }
